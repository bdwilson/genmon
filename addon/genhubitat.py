...
# -------------------------------------------------------------------------------
#    FILE: genhubitat.py
# PURPOSE: Native Hubitat integration backend for genmon.
#          Exposes a REST + WebSocket API that the Hubitat integration connects to.
#
#  AUTHOR: bdwilson
#    DATE: 06-2026
#
...
# -------------------------------------------------------------------------------
class GenHubitat(MySupport):
    """REST + WebSocket API server bridging genmon to Hubitat."""
    ...
    def __init__(
        ...
        super(GenHubitat, self).__init__()
        ...
        try:
            ...
            self.Threads["HubitatPollingThread"] = MyThread(
                self._polling_thread, Name="HubitatPollingThread", start=False
            )
            self.Threads["HubitatPollingThread"].Start()

            self.Threads["HubitatServerThread"] = MyThread(
                self._server_thread, Name="HubitatServerThread", start=False
            )
            self.Threads["HubitatServerThread"].Start()

        except Exception as e1:
            self.LogErrorLine("Error in GenHubitat init: " + str(e1))

    # ---------- Configuration --------------------------------------------------
    def _load_config(self):
        try:
            config = MyConfig(
                filename=os.path.join(self.ConfigFilePath, "genhubitat.conf"),
                section="genhubitat",
                log=self.log,
            )
            self.ServerPort = config.ReadValue("port", return_type=int, default=9084)
            ...
            self.ZeroconfEnabled = config.ReadValue(
                "zeroconf_enabled", return_type=bool, default=False
            )
    ...
    def _ensure_api_key(self):
        ...
        try:
            ...
            config_path = os.path.join(self.ConfigFilePath, "genhubitat.conf")
            config = MyConfig(
                filename=config_path, section="genhubitat", log=self.log
            )
            config.WriteValue("api_key", self.ApiKey)
            self.LogError("Generated new API key for genhubitat")
        except Exception as e1:
    ...
    def _matches_controller(entity_def, controller_type):
        ...
        if "custom" in allowed and controller_type not in GenHubitat._STANDARD_TYPES:
            return True
        return False
    ...
    def _polling_thread(self):
        ...
        while True:
            if self.WaitForExit("HubitatPollingThread", float(self.PollTime)):
                return
            try:
                self._poll_genmon()
            except Exception as e1:
    ...
    def _ensure_tls_cert(self):
        ...
        key_path = os.path.join(self.ConfigFilePath, "genhubitat.key")
        crt_path = os.path.join(self.ConfigFilePath, "genhubitat.crt")
        ...
        try:
            ...
            cn = _sock.gethostname() or "genhubitat"
    ...
    def _server_thread(self):
        try:
            ...
            self.LogError(
                "GenHubitat API server started on port "
    ...
    def _start_zeroconf(self):
        ...
        try:
            ...
            self.ZeroconfInfo = ServiceInfo(
                "_hubitat._tcp.local.",
                f"Hubitat Generator ({hostname})._hubitat._tcp.local.",
            ...
            self.LogError(
                f"Zeroconf: broadcasting _hubitat._tcp on {local_ip}:{self.ServerPort}"
            )
        except Exception as e1:
    ...
    def Close(self):
        self.LogError("GenHubitat shutting down")
        self.Exiting = True
        try:
            self.KillThread("HubitatPollingThread")
        except Exception as e1:
...
if __name__ == "__main__":
    ...
    (
        ...
        loglocation,
        log,
    ) = MySupport.SetupAddOnProgram("genhubitat")
